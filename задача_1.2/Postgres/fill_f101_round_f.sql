...
begin
    ...
    insert 
     ...
     from 
     	(select from_date, to_date, chapter, ledger_account, characteristic, balance_in_rub, balance_in_val, balance_in_total, turn_deb_rub, turn_deb_val,
     			turn_deb_total, turn_cre_rub, turn_cre_val, turn_cre_total, 
 				 sum(case 
	          		when characteristic = 'A' and currency_code in ('643', '810') 
               			then balance_in_rub - turn_cre_rub + turn_deb_rub
		      		when characteristic = 'P' and currency_code in ('643', '810') 
                		then balance_in_rub + turn_cre_rub - turn_deb_rub
		      		else 0
          			end)                            as balance_out_rub,
     			sum(case 
	          		when characteristic = 'A' and currency_code not in ('643', '810') 
                		then balance_in_val - turn_cre_val + turn_deb_val
		      		when characteristic = 'P' and currency_code not in ('643', '810') 
                		then balance_in_val + turn_cre_val - turn_deb_val
              		else 0
     		...
     		from
    			(select  date_trunc('month', i_OnDate)        as from_date,
			     ...
			     group by s.chapter,
			           ...
			           acc_d.char_type,
			          cur.currency_code) sum_t
     	group by from_date, to_date, chapter, ledger_account, characteristic, balance_in_rub, balance_in_val, balance_in_total, turn_deb_rub, turn_deb_val,
	             turn_deb_total, turn_cre_rub, turn_cre_val, turn_cre_total, balance_out_total) sum_t2;
